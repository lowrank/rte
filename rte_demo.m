...
tic;obj = tracer(angle, p, t, n);toc;

%% boundary contribution, which should be in solution. 
bc = zeros(NA, size(p,2));
bc(:, 1:127) = 1.0;

tic;u = obj.boundary_transport(p, t, 5.2*ones(size(p,2),1), bc);toc;
fl = sum(u)/NA;
trisurf(t(1:3,:)', p(1,:), p(2,:), fl', 'EdgeColor', 'none');shading interp;view(2);
